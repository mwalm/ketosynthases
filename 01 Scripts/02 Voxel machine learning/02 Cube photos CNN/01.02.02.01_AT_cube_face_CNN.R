...
# 0. Read the pdb files into a large list/array
{
  ...
  #route <- "/home/q31032mw/Dropbox (The University of Manchester)/Max/17_ML_Project/Rank_1/Aligned_big_set4_snakeless/"
  ...
  Substrates <- read.csv("/home/q31032mw/Dropbox (The University of Manchester)/Max/17_ML_Project/Webscraping/ClusterCAD Webscraping/20230713_AT_SMILES_unified_full_corrections.csv")
...
# Add the layers
model %>%
  layer_conv_2d(filters = 32, kernel_size = c(15, 15), activation = "relu", input_shape = c(cube_boundary, cube_boundary, faces)) %>%
...
# Compile the model
model %>% compile(
  loss = "categorical_crossentropy", # common loss function in image classification
  optimizer = "adam", #adam = adaptive momentum estimation
  metrics = c("accuracy")
)
...
##############################################################################################
### SNAKES ###
route_snakes <- "/home/q31032mw/Dropbox (The University of Manchester)/Max/17_ML_Project/Rank_1/Aligned_AT_SNAKES_mixed/"
files_list_snakes <- list.files(path = route_snakes)

total_data_snakes <- list()

for (i in files_list_snakes){
  name_in_list <- paste0(route_snakes, i)
  pdb <- read.pdb(name_in_list)
  pdb2 <- cbind(pdb[1], as.data.frame(pdb[3]))
  colnames(pdb2) <- gsub(x = colnames(pdb2),pattern = "atom.", replacement = "") 
  total_data_snakes[[i]] <- pdb2[-dcs]
}


  for (i in 1:length(total_data_snakes)) {
    total_data_snakes[[i]][[6]] <-  (total_data_snakes[[i]][[6]]) - min_x2
    total_data_snakes[[i]][[7]] <-  (total_data_snakes[[i]][[7]]) - min_y2
    total_data_snakes[[i]][[8]] <-  (total_data_snakes[[i]][[8]]) - min_z2
  }

  refined_total_data_snakes <- total_data_snakes
  # Apply atom_refactor to coordinates. 
  for (i in 1:length(total_data_snakes)) {
    refined_total_data_snakes[[i]][[6]] <-  round(digits = rounding_number, x = ((total_data_snakes[[i]][[6]])/atom_refactor))
    refined_total_data_snakes[[i]][[7]] <-  round(digits = rounding_number, x = ((total_data_snakes[[i]][[7]])/atom_refactor))
    refined_total_data_snakes[[i]][[8]] <-  round(digits = rounding_number, x = ((total_data_snakes[[i]][[8]])/atom_refactor)) 
    refined_total_data_snakes[[i]][[10]] <-  (total_data_snakes[[i]][[10]])/100
  }
  
  
  final_snake_data <- refined_total_data_snakes
  for (i in 1:length(refined_total_data_snakes)) {
    final_snake_data[[i]] <- final_snake_data[[i]][,-columns_to_ignore]
  }
  
  
  snake1 <- final_snake_data
  ## XY front face ## 
  for (i in 1:length(names(final_snake_data))) {
    data <- as.matrix(final_snake_data[[i]][,1:3])
    coords <- t(apply(data, 1, function(row) as.integer(row * 10^rounding_number)))
    empty_array <- array(data= 0, dim = c(faces, cube_boundary, cube_boundary)) 
  ...
  for (i in 1:length(names(final_snake_data))) {
    ...
    snake1[[i]][3,,] <- empty_array
  }
  # Convert the list of genes to a single array
  SNAKES <- array_reshape(unlist(snake1), dim = c(length(snake1), faces, cube_boundary, cube_boundary))
  
  #predictions
  # Reshape the data
  x_pred <- array_reshape(SNAKES, c(dim(SNAKES)[1], dim(SNAKES)[2], 100, 100))
  x_pred <- aperm(x_pred, c(1, 3, 4, 2))
  
  # Make predictions
  predictions <- model %>% predict(x_pred)
  snake_names <- names(final_snake_data)
  predictions <- round(predictions, digits = 3)
  colnames(predictions) <-  c("mal", "mmal")
  predictions <- cbind(snake_names, predictions)

  # Subtract rows with even numbers from the row above it
  get_odd_numbers <- function(limit) {
    odd_numbers <- c()
    for (i in 1:limit) {
      if (i %% 2 != 0) {
  ...
  odd_numbers <- get_odd_numbers(nrow(predictions))
  
 difference_matrix <- matrix(data = NA, nrow = (nrow(predictions)/2), ncol = 2)
 countr <- 0
   for (i in odd_numbers) {
    i2 <- i+1
    countr <- countr + 1
    mal <- round(as.numeric(predictions[i, 2]) - as.numeric(predictions [(i2), 2]), digits = 3)
    mmal <- round(as.numeric(predictions[i, 3]) - as.numeric(predictions [(i2), 3]), digits = 3)
    difference_matrix[countr, 1] <- mal
    difference_matrix[countr, 2] <- mmal
  }
 colnames(difference_matrix) <-  c("mal", "mmal")
 difference_matrix <- difference_matrix*100L
 difference_matrix <- cbind(snake_names[odd_numbers], difference_matrix)
 
 difference_matrix <- cbind(difference_matrix, rep(as.character("mal"), nrow(difference_matrix)) )
 difference_matrix[grepl(pattern = "mmal", x = difference_matrix[,1]), 4 ] <- "mmal"
 for(i in 1:nrow(difference_matrix)){
   if (grepl(pattern = "mmal", difference_matrix[i, 4]) == TRUE) {
     if(as.numeric(difference_matrix[i,3]) < as.numeric(difference_matrix[i,2])){
       difference_matrix[i,4] <- paste0("Improving prediction: ", difference_matrix[i,2], "%")
     }
     else(difference_matrix[i,4] <- "missed")
   }
   else(
     if(as.numeric(difference_matrix[i,2]) < as.numeric(difference_matrix[i,3])){
      difference_matrix[i,4] <- paste0("Improving prediction: ", difference_matrix[i,3], "%")
     }
      else(difference_matrix[i,4] <- "missed")
   )
 }
 difference_matrix
 print(paste0(sum(grepl(pattern = "Improving", difference_matrix[,4])), "/", nrow(difference_matrix), " improving"))

###############################################################
### Identify which are being predicted incorrectly ###
#########################################################

trained_missed <- model %>% predict(x_train2)
trained_missed <- round(trained_missed)
trained_missed <- cbind(names(x_train), trained_missed, y_train)
missed <- c()
for (i in 1:nrow(trained_missed)) {
  if(trained_missed[i, 2] != trained_missed[i, 4]){
    missed <- c(missed,trained_missed[i,1])}
}

tested_missed <- model %>% predict(x_test3)
tested_missed <- round(tested_missed)
tested_missed <- cbind(names(x_test), tested_missed, y_test)
for (i in 1:nrow(tested_missed)) {
  if(tested_missed[i, 2] != tested_missed[i, 4]){
    missed <- c(missed,tested_missed[i,1])}
}
 
master <- read.csv("/home/q31032mw/Dropbox (The University of Manchester)/Max/17_ML_Project/20230714_missed.csv")
master <- append(master[,2], missed)
write.csv(x= master, file = "/home/q31032mw/Dropbox (The University of Manchester)/Max/17_ML_Project/20230714_missed.csv")
 
 
library(plotly)
my_list <- master
item_counts <- table(my_list)

# Create a bar plot with hover text
plot_ly(
  x = names(item_counts),
  y = item_counts,
  type = "bar",
  text = paste("Item: ", names(item_counts), "<br>Frequency: ", item_counts),
  hoverinfo = "text"
) %>%
  layout(title = "Incorrect labelling frequency", xaxis = list(title = "Genes"), yaxis = list(title = "Frequency"))
