...
# for now, let's call it 3 decimal places. We ought to go up to 7 but that may get a bit too RAM heavy. 
rounding_number <- 2
{
  for (i in 1:length(total_data)) {
...
# 2.  Create and populate a 100x100x100 cube 
{
  ...
  for (i in 1:length(names(x_train))) {
    ...
    empty_array <- array(data= 0, dim = c(cube_boundary, cube_boundary, cube_boundary)) 
    
    for(ii in 1:nrow(coords)){
      empty_array[coords[ii,1], coords[ii,2], coords[ii,3]] <- 1L
    }
    x_train[[i]] <- empty_array
  }
}
...
# 3. a CNN#
  ...
  # Convert list of arrays to a 5D array
  x_train_array <- array_reshape(simplify2array(x_train), c(length(x_train), 100, 100, 100))
  
  # Reshape the input data
  x_train_reshaped <- array_reshape(x_train_array, c(length(x_train), 100, 100, 100, 1))
  ...
  # Add the layers
  model %>%
    layer_conv_3d(filters = 32, kernel_size = c(3, 3, 3), activation = "relu", input_shape = c(100, 100, 100, 1)) %>%
